    ...
    return (
        <section className="min-h-screen bg-neutral-950 text-white flex items-center justify-center pt-24 px-4">
            <div className="w-full max-w-md rounded-2xl border border-neutral-800 bg-neutral-900/70 backdrop-blur p-6 md:p-8">
                ...
                <p className="text-neutral-400 mt-1 mb-6 text-sm">Fill in the information below</p>
                <form className="flex flex-col gap-4" onSubmit={onSubmit}>
                    <label className="text-sm text-neutral-300">Full name</label>
                    <div className="relative">
                        <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400">
                            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8V22h19.2v-2.8c0-3.2-6.4-4.8-9.6-4.8z"/></svg>
                        </span>
                        <input value={fullName} onChange={(e)=>setFullName(e.target.value)} type="text" className="w-full pl-10 pr-3 py-2 rounded-lg bg-neutral-800 text-white border border-neutral-700 outline-none focus:border-amber-500 transition" placeholder="Full name" required />
                    </div>
                    ...
                    <button disabled={submitting} type="submit" className="mt-2 w-full bg-amber-500 text-black font-semibold py-2.5 rounded-lg hover:bg-amber-400 transition disabled:opacity-60">{submitting ? "Creating..." : "Create account"}</button>
                    ...
                    <button type="button" onClick={signUpWithGoogle} disabled={submitting} className="w-full bg-white text-black font-semibold py-2.5 rounded-lg hover:bg-neutral-200 transition disabled:opacity-60 flex items-center justify-center gap-2">
                ...
                <div className="mt-4 text-neutral-300 text-sm">
                    Already have an account? <Link className="text-amber-400" to="/login">Sign in</Link>
                </div>
            </div>
        </section>
    );
